...
export default function Home() {
  ...
  const [openRemoveModal, setOpenRemoveModal] = useState(false); // New state for Remove Modal
  ...
  const removeItemCompletely = async (item) => {
    ...
    await deleteDoc(docRef);
    await updateInventory();
    setOpenRemoveModal(false); // Close the modal after removing
  };
  ...
  const handleOpenRemoveModal = () => setOpenRemoveModal(true); // Open remove modal
  const handleCloseRemoveModal = () => setOpenRemoveModal(false); // Close remove modal
